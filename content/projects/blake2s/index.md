...
This article will mostly focus on the architecture and on the design choices of the implementation I made. The design itself is quickly covered up, and the full codebase can be found in [my github repository](https://github.com/Essenceia/blake2_asic).

### Accessible, Not Easy
The goal of this project was to independently tape out my first ASIC outside of any organized team. This involves, solo: designing, validating, and successfully taping out a fully-featured Blake2s hashing accelerator on the SkyWater 130nm process through the Tiny Tapeout shuttle program.
...
The final chip users can configure which project they want to enable during operation and all other designs will be power gated.
The final chip is sold as part of a dev board, which contains both the TinyTapeout chip and is connected to an RP2040 MCU.
...
A single sky130 Tiny Tapeout project tile is, as the name foreshadows, tiny… About 161x111.52 um, or about as large as to realistically fit 256 bits worth of flip-flops on a good day.
...
The `a`, `b`, `c`, and `d` indexes used to read and write the `v` vector when calling G_{n} Gn​ do not overlap with G_{n+1} for n \in \{0,1,2,4,5,6\}. Though, there is overlap for n=3, d=15 and n=7,b=4.
...
Wait for data_ready_o to be asserted.
Wait for data to be available in the FIFO.
Synchronize with the clock.
Stream out data until the block transfer is complete.
Repeat.
